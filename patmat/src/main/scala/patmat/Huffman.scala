...
object Huffman {
   ...
   */
  def until(breaker: List[CodeTree] => Boolean,
    combiner: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): CodeTree = singleton(trees) match {
    case true => trees(0)
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    table.find(pair => pair._1 == char) match {
